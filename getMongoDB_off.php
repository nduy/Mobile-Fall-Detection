<?php
   ...
   if (in_array($colName, $currentCollections)) {
		...
		$endPoint = new MongoDate($lastSec+1, $lastUSec);
...
//        $resultCursor=$collection-> find(array("indexTime" => array('$gt' => $past), "sensorName" => $sname));
        if ($lastSec) {
           $resultCursor = $collection->find(array("indexTime" => array('$gt' => $past, '$lte' => $endPoint)))->sort(array("sensorName" => 1, "indexTime" => 1));
        }
        else {
           $resultCursor = $collection->find()->sort(array("indexTime" => 1))->limit(1); // get the oldest value
        }
